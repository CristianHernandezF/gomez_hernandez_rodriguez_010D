...
public class RegistroDoctorController {
    ...
    @PostMapping("")
    public ResponseEntity<RegistroDoctor> registrarDoctor(@Valid @RequestBody RegistrarDoctorRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(doctorService.registrarDoctor(request));
    }

    @GetMapping("/todos")
    public List<RegistroDoctor> listarDoctores() {
        return doctorService.listarTodos();
    }

    @GetMapping("/{runDoctor}")
    public ResponseEntity<RegistroDoctor> obtenerDoctor(@PathVariable String runDoctor) {
        return ResponseEntity.ok(doctorService.obtenerPorRun(runDoctor));
    }

    @PutMapping("/{runDoctor}")
    public ResponseEntity<RegistroDoctor> actualizarDoctor(
